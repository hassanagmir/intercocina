...
<section class="px-4 py-20 md:max-w-6xl md:mx-auto">
    <div class="relative z-10 mx-auto px-6 md:max-w-2xl lg:max-w-5xl lg:px-12">
        <div class="mx-auto text-center lg:w-4/5">
            <h1 class="text-xl font-black text-red-600 dark:text-white sm:text-6xl">
                ...
                <div class="mx-auto block w-max">
                    <div class="relative block pb-2">
                        <span class="absolute inset-0 z-[1] block bg-gradient-to-b from-red-600 via-red-600 to-transparent bg-clip-text text-transparent dark:from-white dark:via-white">Templates</span>
                        <span class="absolute inset-0 block bg-gradient-to-l from-[#b6b9b8] to-[#cfd0d1] bg-clip-text text-transparent">Templates</span>
    ...
    <div class="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-4 mt-8">
        @foreach ($events as $event)
            <div class="rounded-xl bg-white border overflow-hidden">
                <a class="relative flex h-96 overflow-hidden bg-gray-300" href="{{ route('event.show', $event->slug) }}">
                    <img class="object-cover w-full h-full border-b" src="{{ $event->getFirstMediaUrl() }}" alt="product image" />
                </a>
