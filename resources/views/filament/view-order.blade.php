        ...
        <div class="md:gap-6 lg:flex lg:items-start xl:gap-8">
                    ...
                    <x-filament::section>
                      <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                        ...
                        <a href="{{ route('product.show', $item->product->slug) }}" class="shrink-0 md:order-1">
                            <img class="h-20" src="{{ url(config('app.storage'), $item->product->images?->first()->image) }}" alt="{{ $item->product->name }}">
                        </a>
                        <!-- Quantity control -->
            ...
            <div class="mx-auto mt-6 max-w-5xl flex-1 space-y-6 lg:mt-0 lg:w-full">
              <x-filament::section>
                ...
                <p class="text-xl font-semibold text-gray-900 dark:text-white mb-3">{{ $this->record?->user?->name }} ({{  $this->record?->user->code}})</p>  
                @else
                <p class="text-xl font-semibold text-gray-900 dark:text-white mb-3">{{ $this->record?->user?->full_name }} ({{  $this->record?->user->code}})</p>
                @endif
                <div class="space-y-4">
                  <div class="space-y-6">
